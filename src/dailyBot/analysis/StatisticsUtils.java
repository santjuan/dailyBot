...
public class StatisticsUtils 
{
	...
	public static double calculateATR(Pair pair, int period, long closingDate)
	{
		...
		for(PairHistory actual : map.values())
		{
			...
			values.add(Math.max((actual.high - actual.low) * pair.pairMultiplier(), Math.max(Math.abs(actual.high - lastClose) * pair.pairMultiplier(), Math.abs(actual.low - lastClose) * pair.pairMultiplier())));
			lastClose = actual.close;
		}
