...
public class SignalProvider extends XMLPersistentObject
{
	...
	public enum SignalProviderId
	{
		...
		DAILYBOTSSIEURO(
				new FilterFactory()
				{
					...
					public Filter[] getFilters(SignalProviderId id) 
					{
						return new Filter[]{"octave".equals(DailyProperties.getProperty("dailyBot.model.SignalProvider.DAILYBOTSSIEURO.filter")) ? new OctaveFilter(id) : new BasicFilter()};
					}	
				}
				,
				new BrokerFactory()
		...
		DailyThread persistenceThread = new DailyThread(new DailyRunnable()
		{
			...
			public void run() 
			{
				...
				while(true)
				{
					...
					{
						DailyThreadInfo.registerUpdate(id.toString() + " persistence", "State", "error storing in db " + e + " " + e.getMessage());
						throw e;						
					}
					finally
					{
					...
					else
					{
						...
						{
							DailyThreadInfo.registerUpdate(id.toString() + " checker", "State", "error checking brokers " + e + " " + e.getMessage());
							throw e;
						}
						finally
						{
	...
	public void processSignal(StrategySignal signal, boolean hit)
	{
			if(hit)
			{
				if(signal.getUniqueId(id.toString()) == 0L)
					DailyLog.logError("Senal con par: " + signal.getPair() + ", estrategia: " + signal.getStategyId() + ", proveedor " + id + " no existe y se intento cerrar.");
				else
				{
					for(Broker broker : brokers)
			...
			{
				if(signal.getUniqueId(id.toString()) != 0L)
					DailyLog.logError("Senal con par: " + signal.getPair() + ", estrategia: " + signal.getStategyId() + ", proveedor " + id + " ya existe y se intento abrir otra vez.");
				else
				{
					if(!filterAllow(new SignalHistoryRecord(signal.getStategyId(), signal.getPair(), signal.isBuy(), signal.getStartDate())))
						for(Broker broker : brokers)
	...
	public void openActive(StrategyId strategyId, Pair pair)
	{
		...
		if((toOpen.getUniqueId(id.toString()) == 0L) || (toOpen == null) || (toOpen.getUniqueId(id.toString()) == 0L))
			DailyLog.logError("Senal con par: " + pair + ", estrategia: " + strategyId + ", proveedor " + id + " no estaba abierta y se intento reabrir.");
		else
		{
	...
	public void stopped(StrategySignal signal) 
	{
		...
		{
			DailyLog.logInfoWithTitle("rangos", id + " cerrando por stop " + signal.getStategyId() + ", " + signal.getPair() + " Precio entrada: " + signal.getEntryPrice() + " Stop: " + signal.getStop() + " Es compra: " + signal.isBuy());
			for(Broker broker : brokers)
				if(broker.getUniqueId(signal) != 0)
					broker.closeSignal(signal, signal.getStategyId(), signal.getPair(), signal.isBuy());
			...
			DailyLog.logError("Senal con par: " + signal.getPair() + ", estrategia: " + signal.getStategyId() + ", proveedor " + id + " no existe y se intento cerrar (toco stop).");
	}
		...
		if(currentActive && !newActive)
		{
			if(signal != null && signal.getUniqueId(strategyId.toString()) != 0)
			{
				for(Broker broker : brokers)
					...
					{
						broker.closeSignal(signal, signal.getStategyId(), signal.getPair(), signal.isBuy());
						DailyLog.logError("Proveedor " + this.id + ", estrategia: " + strategyId + ", par: " + pair + " estaba abierta y se desactivo, cerrada manualmente.");
					}
