		...
		try
		{
			...
			if(zeroBid)
			{
				if(isCorrect(bid) && isCorrect(ask) && pairSpread(bid, ask, this) <= 75)
				{
					...
					errorMessage = "Error en Par inicializando " + toString() + ", bid anterior: " + currentBid + ", bid nuevo: " + bid + ", ask anterior: " + currentAsk + ", ask nuevo: " + ask;
				startNumber++;
			}
			else
			{
				if(Math.abs(differenceInPips(bid, true)) <= 200 && Math.abs(differenceInPips(ask, false)) <= 200 && pairSpread(bid, ask, this) <= 75)
				{
					currentBid = bid;
					currentAsk = ask;
				}
				...
				{
					errorMessage = "Error en par " + toString() + ", bid anterior: " + currentBid + ", bid nuevo: " + bid + ", ask anterior: " + currentAsk + ", ask nuevo: " + ask;
				}
			}
		}
		finally
	...
	public String checkSignals()
	{
		...
		try
		{
			...
			for(StrategySignal s : signals)
				debug += s.getStategyId().toString() + " " + s.getPair().toString() + " " + s.getEntryPrice() + " " + s.isBuy() + " " + s.getLow() + " " + s.getHigh() + "\n";
			return debug;
		}
		finally
		{
