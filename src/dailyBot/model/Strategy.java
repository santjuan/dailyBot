...
{	
	...
	public void processSignalChange(Pair pair, boolean hit, boolean buy, int lotNumber, double entryPrice, StrategySignal affected) 
	{
		write.lock();
		try
		{
	...
	public void checkStops()
	{
		...
		try
		{
			for(StrategySignal affected : signals)
			{
				...
				if(affected.isBuy())
				{
					if((affected.getPair().getCurrentPrice(true) < affected.getStop()) && (affected.getPair().getCurrentPrice(false) < affected.getStop()) && ((affected.getLotNumber() < 4) || (affected.getStop() > affected.stopDaily())))
					{
				...
				{
					if((affected.getPair().getCurrentPrice(false) > affected.getStop()) && (affected.getPair().getCurrentPrice(true) > affected.getStop()) && ((affected.getLotNumber() < 4) || (affected.getStop() < affected.stopDaily())))
					{
						if(!affected.isStopTouched())
						{
							for(SignalProviderId signalProviderId : SignalProviderId.values())
