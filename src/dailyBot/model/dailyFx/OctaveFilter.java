...
public class OctaveFilter extends ExternalProcessFilter
{
	...
	{
		this.percentage = Double.parseDouble(DailyProperties.getProperty("dailyBot.model.dailyFx.OctaveFilter." + id.toString() + ".percentage"));
		this.pips = Integer.parseInt(DailyProperties.getProperty("dailyBot.model.dailyFx.OctaveFilter." + id.toString() + ".pips"));
	...
	private static synchronized void write(SignalHistoryRecord record, double percentage, int pips)
	{
		...
		try 
		{
			...
			{
				if((currentTime - currentRecord.openDate) <= (12L * 30L * 24L * 60L * 60L * 1000L) && currentRecord.id.equals(record.id) && currentRecord.pair.equals(record.pair) && (currentRecord.buy == record.buy))
				{
	...
	public static synchronized String process(SignalHistoryRecord record, double percentage, int pips, TreeMap <SignalHistoryRecord, Boolean> map)
	{
		...
		try 
		{
			Process process = Runtime.getRuntime().exec(DailyProperties.getProperty("dailyBot.model.dailyFx.OctaveFilter.command"));
			process.waitFor();
