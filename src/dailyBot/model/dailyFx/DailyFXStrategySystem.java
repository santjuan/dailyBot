...
public class DailyFXStrategySystem extends StrategySystem
{
	...
	{
		strategies = new StrategyId[] {StrategyId.BREAKOUT1, StrategyId.BREAKOUT2, StrategyId.RANGE1, StrategyId.RANGE2, StrategyId.MOMENTUM1, StrategyId.MOMENTUM2};
	}
			...
			public void run() 
			{
				...
				while(true)
				{
					...
					{	
						...
						{
							System.gc();
							DailyThreadInfo.registerUpdate("DailyFX updater", "State", "error processing " + e + " " + e.getMessage());
							errorCount++;
		...
		DailyThread persistenceThread = new DailyThread(new DailyRunnable()
		{
			public void run() 
			{
				while(true)
				{
					...
					{
						DailyThreadInfo.registerUpdate("DailyFX persistence", "State", "error storing " + e + " " + e.getMessage());
						throw e;
					}
					finally
					{
	...
	protected void process(String[] read)
	{
		try
		{
			...
			{
				DailyThreadInfo.registerUpdate("DailyFX updater", "Processing state current signal", "processing signal: " + signal);
				...
				if((affected = current.hasPair(signal.getPair())) != null)
				{
					if(signal.isBuy() != affected.isBuy())
					{
						...
						current.processSignalChange(signal.getPair(), false, signal.isBuy(), signal.getLotNumber(), affected.getEntryPrice(), affected);
						changed = true;
						DailyLog.logInfoWithTitle("rangos", "Cambio: " + affected + " por: " + signal);
					}
					if(affected.getLotNumber() > signal.getLotNumber())
					{
						current.processSignalChange(signal.getPair(), true, false, affected.getLotNumber() - signal.getLotNumber(), affected.getEntryPrice(), affected);
						changed = true;
						if(affected.isBuy())
						{
				...
				{
					current.processSignalChange(signal.getPair(), false, signal.isBuy(), signal.getLotNumber(), signal.getEntryPrice(), affected);
					changed = true;
					DailyLog.logInfoWithTitle("rangos", "Agregando: " + signal);
				}
				DailyThreadInfo.registerUpdate("DailyFX updater", "Processing state current signal", "signal processed: " + signal);
			}
			for(StrategyId strategyId : strategies)
			{
				Strategy current = strategyId.strategy();
				DailyThreadInfo.registerUpdate("DailyFX updater", "Processing state", "processing strategy, checking stops: " + strategyId);
				current.checkStops();
				List <StrategySignal> signalsList = current.duplicateSignals();
				for(StrategySignal senal : signalsList)
				{
					DailyThreadInfo.registerUpdate("DailyFX updater", "Processing state existing signals", "processing existing signal: " + senal);
					boolean found = false;
