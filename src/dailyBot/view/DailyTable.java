...
public class DailyTable extends JFrame
{
	...
	private void fillShowArray(StrategyId strategyId)
	{
		...
		for(int i = 0; i < listaE.size(); i++)
		{
			...
			toShow[i][3] = df.format(listaE.get(i).getEntryPrice()) + " " + df.format(listaE.get(i).stopDaily()) + " " + df.format(listaE.get(i).getStop());
			try
			{
				toShow[i][4] = RMIClientMain.connection.getProfitStrategySignal(listaE.get(i).getStategyId().ordinal(), listaE.get(i).getPair().ordinal());
			}
			catch(Exception e)
			{        	
				DailyLog.logRMI(e.getMessage() + " Error haciendo la conexion RMI");
	...
	private void fillShowArray(SignalProviderId signalProviderId) 
	{
		...
		for(int i = 0; i < listaE.size(); i++)
		{
			...
			try 
			{
				StrategySignal esta = RMIClientMain.connection.getStrategySignals(listaE.get(i).getStategyId().ordinal(), listaE.get(i).getPair().ordinal());
				if(esta == null)
				{
				...
				toShow[i][2] = listaE.get(i).getPair().toString();
				toShow[i][3] = df.format(esta.getEntryPrice()) + " " + df.format(esta.stopDaily()) + " " + df.format(esta.getStop());
				toShow[i][4] = RMIClientMain.connection.getProfitStrategySignal(listaE.get(i).getStategyId().ordinal(), listaE.get(i).getPair().ordinal()) + " " + listaE.get(i).getUniqueId("zulutrade-" + signalProviderId.toString());
			} 
			catch(Exception e)
			{
	        	DailyLog.logRMI(e.getMessage() + " Error haciendo la conexion RMI");
