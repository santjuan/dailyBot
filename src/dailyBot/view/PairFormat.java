...
public class PairFormat extends JPanel 
{
	...
	private void initialize() 
	{
		...
		int filas = (Pair.values().length - 1) % 2 == 0 ? (Pair.values().length - 1) / 2 : (Pair.values().length - 1) / 2 + 1;
	...
	private JCheckBox darBoton(Pair p) 
	{
		...
		try 
		{
			nuevo.setSelected(RMIClientMain.connection.getActiveSignalProvider(signalProviderId.ordinal(), strategyId.ordinal(), p.ordinal()));
		} 
		catch (RemoteException e) 
		{
        	DailyLog.logRMI(e.getMessage() + " Error haciendo la conexion RMI");
	...
	private void configurar(final Pair par, JCheckBox box)
	{
    	box.addActionListener(new ActionListener()
    	{
			...
			public void actionPerformed(ActionEvent e) 
			{
				...
				{
					boolean activar = ((AbstractButton) e.getSource()).isSelected();
					RMIClientMain.connection.setActiveSignalProvider(signalProviderId.ordinal(), strategyId.ordinal(), par.ordinal(), activar, false);						
					if(activar && RMIClientMain.connection.getOpenSignalProvider(signalProviderId.ordinal(), strategyId.ordinal(), par.ordinal()))
					{
						...
						RMIClientMain.connection.setActiveSignalProvider(signalProviderId.ordinal(), strategyId.ordinal(), par.ordinal(), activar, abrir);
					}
				} 
				catch (RemoteException e1) 
				{
