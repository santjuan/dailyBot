...
public class IndicatorChart extends JPanel
{
	...
	public void updateChart(Range range, Indicator indicator) 
	{
	    ...
	    for(SignalHistoryRecord r : records)
	    {
	    	...
	    	if(unico)
	    	{
		    	if(range.isInside(indicator.calculate(r), r.buy) || (numero == 2 && range.isInside(indicator.calculate(r), !r.buy)))
		    	{
		    		nTransacciones++;
		    		acum += r.profit;
	    ...
	    JFreeChart chart = ChartFactory.createScatterPlot(indicator + " vs Ganancia ", indicator.toString(), "Ganancia", dataset, PlotOrientation.VERTICAL, false, false, false);
