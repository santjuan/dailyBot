...
public class DailyBotMain
{
	...
	private static Class <?> [] systemClasses =
	{
		DailyFXStrategySystem.class
	};
			...
			public void run() 
			{
				while(true)
				{
					...
					try
					{
						...
						{
							DailyThreadInfo.registerUpdate("VIX updater", "State", "error loading: " + e + " " + e.getMessage());
							throw e;
						}
						finally
						{
					...
					catch(Exception e)
					{
						...
						DailyThreadInfo.registerUpdate("SSI-VIX updater", "State", "error in loop: " + e + " " + e.getMessage());
						DailyThreadInfo.closeThreadLoop("SSI-VIX updater");
					}
					setLastUpdate(System.currentTimeMillis());
				}
				...
				while(true)
				{
					try
					{
						...
						for(Pair pair : Pair.values())
						{
							...
							{
								DailyThreadInfo.registerUpdate("Pair high-low monitor", "State", "error updating pair: " + pair + ", " + e + " " + e.getMessage());
								throw e;
							}
						}
					...
					catch(Exception e)
					{
						...
						DailyThreadInfo.registerUpdate("Pair high-low monitor", "State", "error updating pairs: " + e + " " + e.getMessage());
					}
					finally
					{
			...
			public void run() 
			{
				...
				while(true)
				{
					...
					catch(Exception e)
					{
						...
						DailyThreadInfo.registerUpdate("Chat ping", "State", "error sending ping " + e + " " + e.getMessage());
					}
					finally
					{
	...
	{
		System.setProperty("java.rmi.server.codebase", "file:" + DailyProperties.getProperty("dailyBot.control.DailyBotMain.dailyBotFolder") + "/bin/");
		System.setProperty("java.security.policy", "file:"  + DailyProperties.getProperty("dailyBot.control.DailyBotMain.dailyBotFolder") + "/server.policy");
	}
	...
	public static void main(String [] args) throws IOException
	{
		...
		DailyLog.logInfo("Iniciando operaciones automaticamente: " + current.get(Calendar.DAY_OF_MONTH) + "/" + (current.get(Calendar.MONTH) + 1) + "/" + current.get(Calendar.YEAR) + " " + current.get(Calendar.HOUR_OF_DAY) + ":" + current.get(Calendar.MINUTE) + ":" + current.get(Calendar.SECOND) + "." + current.get(Calendar.MILLISECOND));
		loadProperties();
		if(inLinux)
    	{
			try 
	        {
				if("true".equals(DailyProperties.getProperty("dailyBot.control.DailyBotMain.RMI")))
				{
		            ...
		            Registry registry = LocateRegistry.getRegistry(DailyProperties.getProperty("dailyBot.vista.VentanaPrincipal.direccionRMI"));
