...
public class ZulutradeConnection implements Broker
{
	private class InstantZulutradeConnection
	{
		...
		private InstantZulutradeConnection(SignalProviderId id)
		{
			...
			ai = new ZuluTradeAuthenticationInfo(DailyProperties.getProperty("dailyBot.control.connection.zulutrade.ZulutradeConnection." + id + ".username"), DailyProperties.getProperty("dailyBot.control.connection.zulutrade.ZulutradeConnection." + id + ".password"));
	...
	public class ZulutradeSignal
	{
		...
		{
			return "ZulutradeSignal [isError=" + isError + ", errorCode="
					+ errorCode + ", errorMessage=" + errorMessage
					+ ", uniqueId=" + uniqueId + ", dateOpened=" + dateOpened
					+ ", currency=" + currency + ", isBuy=" + isBuy
					+ ", entryPrice=" + entryPrice + ", stop=" + stop
					+ ", limit=" + limit + "]";
		}	
	}
	...
	public ZulutradeSignal[] listOpenSignals() throws ZulutradeException
	{
		...
		{		
			...
			for(NewMarketTradeResponseMessage marketTrade : new InstantZulutradeConnection(id).tradingGateway.getOpenTrades().getOpenPositions())
			{
				try
				{
	...
	public boolean changeLimit(long id, double newLimit) throws ZulutradeException
	{
		try
		{
			...
			UpdateValueResponseMessage updateResponse = new InstantZulutradeConnection(this.id).tradingGateway.updateLimit(currency.toString().substring(0,  3) + "/" + currency.toString().substring(3), signal.isBuy, 1, id + "", newLimit);
			if(updateResponse == null)
				throw new ZulutradeException(new Exception("Null change limit response for id: " + id));
			if(!updateResponse.isSuccess())
				DailyLog.logError("Not possible to change limit: " + updateResponse.getErrorCode() + " " + updateResponse.getErrorMessage());
	...
	public boolean changeStop(long id, double newStop) throws ZulutradeException
	{
		try
		{
			...
			Pair currency = signal.currency;
			UpdateValueResponseMessage updateResponse = new InstantZulutradeConnection(this.id).tradingGateway.updateStop(currency.toString().substring(0,  3) + "/" + currency.toString().substring(3), signal.isBuy, 1, id + "", newStop);
			if(updateResponse == null)
				throw new ZulutradeException(new Exception("Null change stop response for id: " + id));
			if(!updateResponse.isSuccess())
				DailyLog.logError("Not possible to change stop: " + updateResponse.getErrorCode() + " " + updateResponse.getErrorMessage());
			return updateResponse.isSuccess();
		}
		catch(Exception e)
		{
	...
	public ZulutradeSignal openTrade(Pair currency, boolean isBuy, double limit, double stop) throws ZulutradeException
	{
		try 
		{
			...
			ZulutradeSignal answer = new ZulutradeSignal(new InstantZulutradeConnection(id).tradingGateway.openMarket(currency.toString().substring(0,  3) + "/" + currency.toString().substring(3), 1, isBuy, currency.getCurrentPrice(isBuy), uniqueId + ""));
			if(answer.isError)
			...
			if(!changeLimit(answer.uniqueId, limit))
				throw new ZulutradeException(new Exception("Error setting initial limit, trade possibly opened without limit"));
	...
	public boolean closeTrade(long id) throws ZulutradeException
	{
		try
		{
			...
			TradeClosedResponseMessage closedResponse = new InstantZulutradeConnection(this.id).tradingGateway.closeMarket(signal.currency.toString().substring(0,  3) + "/" + signal.currency.toString().substring(3), 1, signal.isBuy, signal.uniqueId + "", signal.currency.getCurrentPrice(signal.isBuy));
	...
	public void checkConsistency()
	{
		try
		{
			...
			{
				if(signal != null && signal.getUniqueId("zulutrade-" + id.toString()) != 0 && !ids.contains(signal.getUniqueId("zulutrade-" + id.toString())))
				{
				...
				{
					if(signal.getPair().differenceInPips(signal.getStop(), signal.isBuy()) < 0 && Math.abs(signal.getStop()) > 1e-5d)
					{
						closeSignal(signal, signal.getStategyId(), signal.getPair(), signal.isBuy());
						DailyLog.logError("Senal: " + signal + " toco stop, cerrando. Precio actual = " + signal.getPair().getCurrentPrice(signal.isBuy()));
						if(signal != null)
	...
	{
		return dailySignal.getStategyId() + " " + (dailySignal.isBuy() ? "Compra" : "Venta") + " " + dailySignal.getLotNumber() + " Lotes de " + zuluSignal.currency + " a: " + zuluSignal.entryPrice + " Stop: " + zuluSignal.stop; 
	}
	...
	public boolean openSignal(UniqueIdSignal affected, StrategyId strategyId, Pair pair, boolean buy)
	{
		try
		{
			...
			ZulutradeSignal signal = openTrade(pair, buy, pair.getCurrentPriceMinus(-300, buy), pair.getCurrentPriceMinus(75, buy));
	...
	public boolean closeSignal(UniqueIdSignal affected, StrategyId strategyId, Pair pair, boolean buy)
	{
		try
		{
			DailyLog.logInfoWithTitle("rangos", id + " cerrando senal: " + strategyId + ", " + pair + ", zulutrade: " + findSignalById(affected.getUniqueId("zulutrade-" + id.toString())));
