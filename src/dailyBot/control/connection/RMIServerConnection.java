...
public class RMIServerConnection extends BasicConnection implements RMIConnection
{
	public static class Local implements RMIConnection
	{
		...
		static Boolean[][][] dpActive = new Boolean[SignalProviderId.values().length][StrategyId.values().length][Pair.values().length];
		...
		@Override
		public synchronized void setActiveSignalProvider(int signalProviderId, int strategyId, int pairId, boolean active, boolean open) throws RemoteException 
		{
		...
		@Override
		public synchronized boolean getActiveSignalProvider(int signalProviderId, int strategyId, int pairId) throws RemoteException
		{
			if(dpActive[signalProviderId][strategyId][pairId] != null)
				return dpActive[signalProviderId][strategyId][pairId];
			return dpActive[signalProviderId][strategyId][pairId] = server.getActiveSignalProvider(signalProviderId, strategyId, pairId);
		}
	...
	@Override
	public void setActiveSignalProvider(int signalProviderId, int strategyId, int pairId, boolean active, boolean open) throws RemoteException 
	{
		SignalProviderId.values()[signalProviderId].signalProvider().setActive(StrategyId.values()[strategyId], Pair.values()[pairId], active);
		if(active && open)
			SignalProviderId.values()[signalProviderId].signalProvider().openActive(StrategyId.values()[strategyId], Pair.values()[pairId]);
	}
	
	@Override
	public boolean getActiveSignalProvider(int signalProviderId, int strategyId, int pairId) throws RemoteException
	{
		return SignalProviderId.values()[signalProviderId].signalProvider().getActive(StrategyId.values()[strategyId], Pair.values()[pairId]);
	}
	
	@Override
	public boolean getOpenSignalProvider(int signalProviderId, int strategyId, int pairId) throws RemoteException 
	{
		return SignalProviderId.values()[signalProviderId].signalProvider().isOpen(StrategyId.values()[strategyId], Pair.values()[pairId]);
	}
