					...
					try 
					{
						...
						else
						{
							if(!messageSent && (hour == 19))
							{
								...
								{
									DailyThreadInfo.registerUpdate("Thread monitor", "Thread check state", "checking thread " + dailyThread.getName());
									...
									message += "Ultima actualizacion hace: " + (System.currentTimeMillis() - dailyThread.runnable.getLastUpdate()) + " milisegundos, limite espera: " + dailyThread.runnable.getUpdateInterval() + "\n";
									message += "\n";
								}
						...
						for(DailyThread dailyThread : threads)
						{
							if(dailyThread == threadMonitor)
								continue;
							DailyThreadInfo.registerUpdate("Thread monitor", "State", "checking thread for problems " + dailyThread.getName());
							if(!dailyThread.isAlive())
							{
								DailyLog.logError("Error, hilo termino su ejecucion inesperadamente, reiniciando");
								DailyThreadInfo.registerUpdate("Thread monitor", "State", "thread ended unexpectedly, rebooting " + dailyThread.getName());
								DailyLog.tryReboot();
							}
							if((System.currentTimeMillis() - dailyThread.runnable.getLastUpdate()) > dailyThread.runnable.getUpdateInterval())
							{
								DailyThreadInfo.registerUpdate("Thread monitor", "State", "thread not updated in a lot of time, rebooting " + dailyThread.getName());
								DailyLog.logError("Error, hilo: " + dailyThread.getName() + " no se actualizo en mucho tiempo, intervalo aceptable: " + dailyThread.runnable.getUpdateInterval() + ", ultima actualizacion hace: " + (System.currentTimeMillis() - dailyThread.runnable.getLastUpdate() + " haciendo debug"));
								String message = "";
						...
						DailyLog.logErrorToDisk("Monitor hilos " + number + " corrio sin problemas: " + current.get(Calendar.DAY_OF_MONTH) + "/" + (current.get(Calendar.MONTH) + 1) + "/" + current.get(Calendar.YEAR) + " " + current.get(Calendar.HOUR_OF_DAY) + ":" + current.get(Calendar.MINUTE) + ":" + current.get(Calendar.SECOND) + "." + current.get(Calendar.MILLISECOND));
					}
					catch(Exception e) 
					{
